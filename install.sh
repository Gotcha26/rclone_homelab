...
install() {
    ...
    # Création d'une branche locale main sur le tag installé
    git checkout -b main || {
        echo -e "${YELLOW}⚠️  La branche 'main' existe déjà, elle sera mise à jour pour pointer sur $LATEST_TAG.${RESET}"
        git branch -f main "$LATEST_TAG"
    }
    echo -e "${GREEN}✅  Branche locale 'main' créée sur $LATEST_TAG.${RESET}"
